...
partialSolutions :: [Int] -> Int -> [Int]
partialSolutions ns n = [m | ns' <- choices ns, (e,m) <- results ns']
...
normalize xs y = map abs [x-y | x<- xs]

findSolutions :: [Int] -> Int -> [Expr]
findSolutions xs y = [e | ns<- choices xs, (e,m) <- results ns, abs (m-y)<=w]
                     where w = findSmallest (normalize zs y) 
                           zs = partialSolutions xs y
