...
checkProperColor (x:xs) c g = (not (color x g == c)) && checkProperColor xs c g
...
goodGraph g xs (y:ys) = checkGrundyColor zs g && checkProperColor (getNeighbs y g) (color y g) g
