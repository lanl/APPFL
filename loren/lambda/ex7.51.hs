...
double = "(" ++ pair ++ " (first) " ++ "(" ++ pair ++ "(second )" ++  nil1 ++ "))"
triple = "(" ++ pair ++ " (first) " ++ "(" ++ pair ++ "(second )" ++ "( " ++ pair ++ "(third)" ++ nil1 ++ ")))"
singles = "(" ++ pair ++ twos ++ nil1 ++ ")"
singles1 = "(" ++ pair ++ zero ++ nil1 ++ ")"
doubles = "(" ++ pair ++ ones ++ "(" ++ pair ++ twos ++ nil1 ++ "))" 
triples = "(" ++ pair ++ twos ++ "(" ++ pair ++ threes ++ "(" ++ pair ++ ones ++ nil1 ++ ")))"
--nor $ parse (quadruple ++ second ++ first) = second
...
try21 = "( " ++ null1 ++ "( " ++ triple ++  " ))"
try22 = ifThenElse ++"( " ++ null1 ++ "( " ++ triple ++  " ))"
try23 = ifThenElse ++"( " ++ null1 ++ "( " ++ nil1 ++  " ))"


try24 = ifThenElse ++"( " ++ null1 ++ "( " ++ triple ++  " ))" ++ zero ++ ones
try25 = ifThenElse ++"( " ++ null1 ++ "( " ++ nil1 ++  " ))" ++ zero ++ ones

try26 = "(\\n (" ++ ifThenElse ++"( " ++ null1 ++ "(n) )" ++ zero ++ ones ++ "))" ++  double 
try27 = "(\\n (" ++ ifThenElse ++"( " ++ null1 ++ "(n) )" ++ zero ++ ones ++ "))" ++  nil1 

try28 = "(\\n (" ++ ifThenElse ++"( " ++ null1 ++ "(n) )" ++ zero ++ ones ++ "))" 
--nor $ parse(try28 ++ double) = (\f.(\x.f x))
--nor $ parse(try28 ++ nil1) = (\f.(\x.x))

try29 = "(\\r (\\n (" ++ ifThenElse ++"( " ++ null1 ++ "(n) )" ++ zero ++ "(" ++ add1 ++ ones ++ "(" ++ "(r)" ++ "(" ++ second ++ "n)" ++  "))" ++ ")))" 

try30 = "(\\r (\\n (" ++ ifThenElse ++"( " ++ null1 ++ "(n) )" ++ zero ++ "(" ++ add1 ++ ones ++ "(" ++ "(r)" ++ "("++ "n" ++ second ++")" ++  "))" ++ ")))" 
helper x =  last $  startEvalNormOrder $ parse (fix ++ try30 ++ x)
listLength x = startLambToInt $ last $  startEvalNormOrder $ parse (fix ++ try30 ++ x)

try31 = "(\\r (\\n (" ++ ifThenElse ++"( " ++ null1 ++ "(n) )" ++ zero ++ "(" ++ add1 ++ "( n" ++ first ++ ")" ++ "(" ++ "(r)" ++ "("++ "n" ++ second ++")" ++  "))" ++ ")))" 

listSum x = startLambToInt $ last $  startEvalNormOrder $ parse (fix ++ try31 ++ x)
