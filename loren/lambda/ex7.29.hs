--NORMAL/APPLICATIVE ORDER EVALUATION
...
evaluateApplicativeOrder expression = snd (appEval(fvs,expression))

appEval :: ([String],Expr) -> ([String],Expr) 
appEval (ws,(Var x)) = (ws,(Var x)) 
appEval (ws,(Comb (Abs (Var x) y) z)) | snd (betaReduce1 ws y (Var x) z) == (Comb (Abs (Var x) y) z) = (ws,(Comb (Abs (Var x) y) z))
                                      | r && s = appEval (fst (betaReduce1 ws y (Var x) z), snd (betaReduce1 ws y (Var x) z))
                                      | r = appEval (fst (appEval (ws,z)), (Comb (Abs (Var x) y) (snd (appEval (ws,z)))))
                                      | otherwise = (fst (appEval (ws,y)), (Comb (Abs (Var x) (snd (appEval (ws,y)))) z))
                                        where r = (Comb (Abs (Var x) y) z) == (Comb (Abs (Var x) (snd (appEval (ws,y)))) z)
                                              s = (Comb (Abs (Var x) y) z) == (Comb (Abs (Var x) y) (snd (appEval (ws,z))))
 
appEval (ws,(Abs x y)) | (Abs x (snd (appEval (ws,y)))) == (Abs x y) = (ws,(Abs x y)) 
                       | otherwise = appEval (abstraction4 (ws,Abs x y))        
appEval (ws,(Comb x y)) | (Comb (snd (appEval (ws,x))) (snd (appEval (ws,y)))) == (Comb x y) = (ws,(Comb x y))
                        | otherwise = appEval (abstraction5 (ws,Comb x y))


abstraction4 :: ([String],Expr) -> ([String],Expr)
abstraction4 (ws,(Abs x y)) =  ((fst r),(Abs x (snd r)))
                                where r = (appEval (ws,(Abs x y)))
abstraction4 (_,_) = error "Here 8"

abstraction5 :: ([String],Expr) -> ([String],Expr)
abstraction5 (ws,(Comb x y)) = (fst s, Comb (snd r) (snd s))
                               where r = (appEval (ws, x))
                                     s = (appEval (fst r, y))
abstraction5 (_,_) = error "Here 9"
