...
data Defn = ODefn {bnd :: Var, oexp :: Exp, mmtype :: Maybe Monotype}
          | DDefn {mtyp :: Monotype, dcons :: [Constr]}
          | TDefn {bnd :: Var, mtyp :: Monotype}
          deriving (Show, Eq)
...
-- else fold on the uniques, accumulating the groups (hence reverse) and the unsorted
  ...
  where ffun (groups, xs) it = let (matches, rest) = partitionBy (f it) xs
                               in (matches:groups, rest)
        partitionBy f = foldr (\x (y,n)-> if f x then (x:y,n) else (y,x:n)) ([],[])
...
subNamesEx ls e = case e of
  ...
  ECs{eexp, cls} ->
    ...
    in  e { eexp = subNamesEx ls' eexp,
            cls = map (\(p,e)->(p,subNamesEx ls' e)) cls }
