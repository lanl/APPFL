...
tuplify = parens . hsep . punctuate comma
tuplConstraint _ 0 = empty
tuplConstraint f i = parens (binaryHang (<> comma) (map f (vars i empty))) <+> text "=>"
vars i suf = map ((<> suf) . text) $ take i allNameStrings
tuplTycon i = tuplify $ vars i empty
binaryHang _ [] = empty
binaryHang _ [e] = e
binaryHang sepf xs =
  let n = length xs `quot` 2
      (left, right) = splitAt n xs
      (ldoc, rdoc)  = (binaryHang sepf left, binaryHang sepf right)
  in hang ((if isEmpty rdoc then id else sepf) ldoc)
     0 rdoc

mkTuplInst clas implF elemOp combOp unitId i =
  hang (hang (text "instance" <+> tuplConstraint (clas <+>) i) 9 -- length "instance "
         (clas <+> tuplTycon i <+> text "where"))
  2 (impl i)
  where
    impl i   = hang implF 2
               (hang (hang (tuplify $ vars i $ int 1) 0
                      (tuplify $ vars i $ int 2))
                 0
                equals <+> rhs i)
    rhs 0   = unitId
    rhs i   = binaryHang (<+> combOp) $
