...
deriveTuplEqStr n = show $ 
  ...
  where mkInst i =
          hang (hang (text "instance" <+> constraint i) 9 -- length "instance "
                (eQ <+> tycon i <+> text "where"))
          2 (impl i)          
        ...
        constraint 0 = empty
        constraint i = parens (binaryHang (<> comma) (map (eQ <+>) (vars i empty))) <+> text "=>"
        vars i suf = map ((<> suf) . text) $ take i allNameStrings
        tycon i = tuplify $ vars i empty
        impl i   = hang (parens equalTo) 2
                   (hang (hang (tuplify $ vars i $ int 1) 0
                          (tuplify $ vars i $ int 2))
                     0
                     equals <+> rhs i)
        rhs 0   = text "True"
        rhs i   = binaryHang (<+> and) $
                  zipWith (\a b -> a <+> equalTo <+> b)
