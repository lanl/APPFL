...
module APPFL.Integer.Type where
...
-- == ((Double) word) * 2 ^ int
-- Maybe this works, just don't want to deal with erroring on foreign calls
-- if possible. [dmr]
encodeDouble# w# i# =
  case int2Double# i# of
    di# -> case word2Double# w# of
      dw# -> dw# *## (2.0## **## di#)
...
-- Maybe this works, just don't want to deal with erroring on foreign calls
-- if possible. [dmr]
-- This may lose accuracy. It's the same as that of encodeDouble# in the foreign
-- call, so perhaps that's fine...
encodeFloat# w# i# =
    case int2Double# i# of
    di# -> case word2Double# w# of
      dw# -> case dw# *## (2.0## **## di#) of
        dres# -> double2Float# dres#
