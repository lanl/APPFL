...
\item Hopefully in the spirit of a student-friendly symposium, for this draft
  ...
  involvement and its influence on our approach, and pointed out
  student contributions in particular.
\end{enumerate}
...
In fact the garbage collector can be invoked at any point in time except when
...
Core, recursively expanding non-recursive let bindings into case bindings.  Thus
it seems that while it makes functions denotationally strict, function arguments
are still evaluated after function calls.
%
...
Research Compiler effort, we will use GHC as the front-end.
