...
Our incarnation of STG, as is our presentation in Figure~\ref{fig:STGsyntax} which
...
Very simple implementations have been prototyped~\cite{explicit-lazy}.  
%
A plug-in for GHC (prior to 8.0) exists for making functions, but not data
structures, strict.~\cite{strict-ghc-plugin}.
%
The Disciplined Disciple Compiler is a full-blown implementation that has been
underway for some time, but the emphasis appears to be on a much more complex
type system for handling stateful computation, and the prospects for its
eventual completion are unknown~\cite{disciple}.
%
...
\subsubsection{Non-strictness as antithetical to parallel evaluation.}  While
compile-time analysis can conservatively estimate when it is safe to strictify
and parallelize, in general the problem is undecidable and the compiler must
default to purely data-dependency-driven evaluation on demand.  This has been
shown to be problematic for three reasons~\cite{Mattson94}.  First, it is
often desired to force parallel evaluation beyond simply to WHNF, requiring
elaborate strictification as previously discussed.  Second is a space
management and runtime issue: because non-strict evaluation is generally
space-intensive, the runtime may be forced to prematurely abort parallel
evaluation when garbage collection is triggered by space constraints.  Third,
when aborting a speculative evaluation, to be able to reclaim space sufficient
bookkeeping needs to be done to be able to roll back the reduction graph.  The
conclusion was that it is hard to amortize the runtime overhead over gains in
parallelism, and thus justify either the runtime or application implementation
efforts in the non-strict context.  In the strict context the runtime can
simply parallelize until resources are saturated, and much less bookkeeping is
required to recover space when speculative evaluations are to be terminated.
...
handling.~\cite{Pointon-GdH-00}.  However, it appears that their scheme for
...
Trinder--speculative parallelism

Trinder's paper (or just web site?) on fault tolerance.

Trinder's student's thesis on fault tolerance.
