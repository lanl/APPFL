...
\emph{EA}.  Black hole (\emph{BLACKHOLE}), signifying a \emph{THUNK} being
evaluated, is only a runtime entity and not actually part of our STG language.
...
STG machinery makes the choice of evaluation strategy easy to change, though
changing code generation can be more efficient by avoiding the unnecessary
creation of \emph{THUNKS} as discussed in Appendix~A.
...
%small \emph{nurseries} in the multithreaded case~\cite{Marlow:2008}. 
...
%Figure~\ref{fig:miniHaskell}.  
Figure~2.  
There is significant overlap with STG resulting
...
differences are given in order of perceived utility, or importance to someone
