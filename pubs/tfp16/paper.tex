...
aspirations~\cite{Marlow:2009}.  To achieve efficient execution they
...
First we establish that this is possible.  By maintaining our own data
and control (continuation) stack all generated C functions can take no
...
gcc and Clang/LLVM will generate jumps instead of calls for C calls.  When using
gcc we can also use a fixed set of machine registers for passing/returning
values, much like GHC.

\subsubsection{A taste of the continuation style.}
Now to implementation, which we discuss for a simple case, then the most complex
case.  To clearly distinguish between a notional C call (a syntactic concept)
and a machine-level call (pushing a return address onto the native C stack) we
will refer to the latter as \emph{call-with-return}, and to a machine-level
tail call as \emph{jump}.

First a detail.  Our implementation of the STG abstract machine has an
abstract register called the \emph{current value register} or \emph{CVR}.
Roughly speaking, it can hold the pointer to what is about to be evaluated or
what was just evaluated.  Now consider the evaluation of \texttt{case e of
  <alts>} if call-with-return is disallowed.  If the compiler cannot determine
that evaluating \texttt{e} does not jump somewhere then there must be some
mechanism to get to the \texttt{alts}.  Our solution is to have a separate C
function for the \texttt{alts}, and before executing the code to evaluate
\texttt{e}, to push a continuation---the address of this function and the free
variables of the \texttt{alts}---onto the stack.  Whatever the code for
\texttt{e} might do, ultimately (in the absence of error or non-termination)
the value of \texttt{e} is in the CVR, and some code somewhere reaches its end
and ``returns''---jumps to the address
in the continuation on the top of the stack, thus entering the code for
\texttt{alts}.  Thus continuations direct where to go to, not how to return
to caller as in a stack for an imperative language.
...
parameter is in the CVR---a pointer to the object to be applied when jumping to \emph{stgApply}.
