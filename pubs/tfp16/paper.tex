...
\begin{enumerate}[before=\itshape]
  ...
  \item Hopefully in the spirit of a student-friendly symposium, for this
    ...
    included perhaps gratuitous detail about process.
  \end{enumerate}
...
with the International Conference on Functional Programming, in its fifth
...
system was designed for fault tolerance.~\cite{Cesarini:2009}.  It has long
...
Steward impressively demonstrates this on a large-scale distributed-memory
...
high-performance computing, and by transitivity in functional programming,
...
itself:  because this is in large part a student project it must be
kept as simple as reasonably possible in various respects.
...
\emph{Core} intermediate representation; 3) analysis and transformation on
Core; 4) transformation of Core to the \emph{STG} intermediate representation;
5) transformation of STG to machine code via one of various imperative
representations (Cmm, LLVM, C).

In short, our plan is to use GHC as our front-end to generate its intermediate
form, STG~\cite{PJ-stockhardware}, then escape to our system from there.
There are various possible ways to accomplish the this: using GHC's option
to dump STG as text, hacking GHC itself and, optionally, using GHC as a
library~\cite{ghc-as-library}.  An alternative would be to start with GHC Core
which we may transition to.
...
The other dimension we vary is whether or not constructors are strict.
Other dimensions are possible, for example whether \texttt{let} bindings
(heap allocations) are constructed on demand, but we restrict ourselves
to function/application and constructor strictness.
...
While performance is not a high priority, we attain proper tail calling
instead of using the trampoline used in earlier versions of
GHC~\cite{PJ-stockhardware}.  Because we maintain our own data and control
(continuation) stack all generated C functions take no explicit arguments and
have return type \emph{void}, and with a single exception in the non-strict
case are invoked in tail call positions, at non-zero optimization levels both
gcc and Clang/LLVM generate jumps instead of calls for C calls.  When using
gcc we also use a fixed set of machine registers for passing/returning values.
...
a deficit or excess number of the arguments.  The \emph{stgApply} function takes
...
For \emph{direct calls}, in which a known function is applied to a number
of arguments equal to its arity, \emph{stgApply} need not be invoked:
the \emph{FUN} is tail-called directly.

Call-with-return becomes even more convenient when considering the various
forms of strictness: when evaluating function or constructor arguments it is
simpler, and arguably more efficient in terms of time (but not space in
general beca) to ``evaluate in place'' than implement the continuation style
to the logical limit.  We are aware, however, that we may need to eliminate
this use of the native C stack so that we can properly unwind the stack in a
more sophisticated parallel runtime when pruning multiple evaluations of the
same thunk, and we have determined that this is a relatively minor exercise:
one possibility is that \emph{stgApply} takes an additional argument
indicating which of its subsequent arguments have already been forced, and at
each invocation if unforced arguments remain, evaluates the next (possibly)
unevaluated argument and tail calls itself, re-using the stack frame with just
this new argument updated.

\emph{It seems that we can dispose with call-with-return of STG functions,
  including stgApply (as opposed to runtime functions) without too much
  effort.  It also seems like this is needed if we want to be able to unwind
  the stack as described in the ``grey hole'' paper.  I'll try to do this this
  week (3/31/16-4/1/16) to verify the feasibility before changing the
  preceding description (which isn't quite right)}
...
\item various analysis and transformation passes \emph{(all by students)}
...
for its eventual completion are unknown~\cite{disciple}.
%
