...
essential in the sense of Bird's \emph{repmin}~\cite{Bird84}, Johnsson's
general attribute grammars~\cite{Johnsson87}, or other instances of
\emph{tying the knot}~\cite{tying-the-knot}.
...
signifying a \emph{THUNK} being evaluated, are only runtime entities and not
actually part of our STG language.
...
object is explicit: this is the operational semantics of \texttt{let} bindings.
...
a deferred expression evaluation is encoded by a \emph{THUNK}.
...
modern C compiler call optimization, the continuation style enables a proper
...
\subsection{Parallel evaluation and other developments}
...
For ease of developing the test suite we have implemented a \emph{mini-Haskell}
front end as detailed in Appendix~A.

We use a continuous integration system augmented with logic to categorize
programs in a large and growing test suite according to which degrees of non-strictness they
require to terminate correctly, and to provide other logging and debugging
information.  This is detailed in Appendix~B.
...
\begin{itemize}
\item STG parsers \emph{(by student)},
...
The concept of strict but still pure dates back to early studies of the lambda
calculus.  In terms of a notionally concrete programming language the first
thorough explications may be of Backus' FP~\cite{Backus:1978} and follow-on
FL implementation~\cite{AikenFL,FLreport89}.
%
Perhaps the first explication of the potential merits of a strict-by-default
...
translated into STG for the back-end.  Here, the differences are given in order
of perceived utility, or importance to someone transitioning from STG to
mini-Haskell.
...
Of course, this complex pattern matching must eventually be transformed into the
primitive, nested case expressions. Luckily, there are well-documented processes
for achieving this, and Philip Wadler's chapter in \emph{The Implementation of
  Functional Programming Languages} \cite{Jones:1987} % doing this right?
on the compilation of this type of pattern matching proved invaluable in
bringing it to mini-Haskell.  Wadler describes the \texttt{match} function that
that, in its simplest form, produces a correct, but potentially large
translation with some redundancy.  This function was implemented with only minor
modifications to fit the shape of mini-Haskell's abstract form.  The
implementation is augmented by some light flow-analysis to eliminate some of the
redundancy and make the resulting code easier to read.  There are further
optimizations suggested by Wadler to address the size of the generated code from
this process (for example, when multiple branches lead to the same large
expression), but the additional complexity outweighs the benefit for a testing
language.


\subsubsection{Less significant features.}

Mini-Haskell provides some smaller conveniences that don't warrant their own
section, but are worth mentioning.
...
\item Primitive operations as functions \\
  ...
  functions, with roughly the same rules.  Partially applied calls are
  translated to generated functions that wrap the actual primitive
