...
functional programming, in various guises.  
%
%A second claimed trend is, given a strict functional semantics, in the
%development of automatically parallelizing runtime systems.
Besides making programs easier to reason about, this enables automatic
parallelization to various degrees.
%
A second claimed trend in both scientific computing and functional programming
is transparent fault tolerance.  
%
...
\item Hopefully in the spirit of a student-friendly symposium, for this draft
  ...
  preparation, involvement, and its influence on our approach, and pointed out
  some specific student contributions.

\item While this \emph{does not qualify} as a student paper, students
...
algorithms for achieving this, and Philip Wadler's chapter in \emph{The
  Implementation of Functional Programming Languages}~\cite{Jones:1987} %
doing this right?  on the compilation of this type of pattern matching proved
invaluable.  Wadler describes the \emph{match} function that which, in its
simplest form, produces a correct, but potentially large translation with some
redundancy.  This function was implemented with only minor modifications to
fit the shape of mini-Haskell's abstract form.  The implementation is
augmented by some light flow-analysis to eliminate some of the redundancy and
make the resulting code easier to read.  There are further optimizations
suggested by Wadler to address the size of the generated code ,for example,
when multiple branches lead to the same large expression, but the additional
