...
We have developed a extensive test suite using the CMake \cite{cmake-book}
CTest tool. The full test suite is automatically run and any failures reported
using a Jenkins continuous integration server \cite{jenkins-book}. There are
test programs for both STG and mini-Haskell. We also run each test with various
levels of garbage collection enabled.

The test suite is partitioned into a number of directories which control what
...
"blackhole" in the case of strict semantics. Another important class of test
programs are those which should fail with a known error (e.g. a parsing error)
for these programs the test suite checks the output against an expected error
regular expression.
