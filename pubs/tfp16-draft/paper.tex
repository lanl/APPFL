...
\item code generator,
\item runtime system including a (serial) garbage collector and instrumentation framework,
\item continuous integration and testing system,
\item extensive test suite, automated testing.
\end{itemize}
Appendixes~\ref{app:minihaskell} and~\ref{app:ghclibrary}
describe the \emph{mini-Haskell} front-end and our approach to using GHC as a front end, respectively.
%
...
a thorough collection of run-time statistics such as number, size, and type of heap
and stack allocations, as well as various garbage collection statistics.  We also
measure cumulative time spent in various operations.  Future work includes processing
these measurement in an other than ad hoc fashion.
...
\vspace{-0.1in}
%\paragraph{Strict but still pure.}
...
% and follow-on FL implementation~\cite{AikenFL,FLreport89}.
%
...
%\paragraph{Strict, pure, auto-parallelizing.}
...
%\paragraph{Fault tolerance for functional programming.}
...
straightforward.
%Trimming Mini-Haskell to fit page limit
%\begin{comment}
