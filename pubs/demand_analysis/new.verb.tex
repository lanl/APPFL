...
\begin{abstract}
  ...
  smoothing over a complicating construction in projection-based strictness
  analysis dating back to Wadler and Hughes.  However, to date no development
  has combined the desirable attributes of all of these approaches, or given
  the details where unboxed types are explicit.  We present the first
  practical, higher-order, polymorphic, forward demand analysis based on
  partial projections, incorporating the notions of partial equivalence
  relations and logical relations, together with a proof of satisfaction of
  the fundamental correctness condition for such analyses.  Such an analysis
  can be used both to improve serial code generation, as well as enable safe
  automatic parallelization, for lazy or non-strict functional languages.
