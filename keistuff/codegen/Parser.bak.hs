...
data Expr = EAtom FVs Atom
          | EFCall FVs Var [Atom]
          | EPrimop FVs Primop [Atom]
          | ELet FVs [Def] Expr
          | ECase FVs Expr [Alt]
            deriving(Eq,Show)

data Alt = ACon FVs Con [Var] Expr
         | ADef FVs Var Expr
           deriving(Eq,Show)

data Obj = FUN {fvs :: FVs, vs :: [Var], e :: Expr}
         | PAP {fvs :: FVs, f :: Var, as :: [Atom]}
         | CON {fvs :: FVs, c :: Con, as :: [Atom]}
         | THUNK {fvs :: FVs, e :: Expr}
         | BLACKHOLE {fvs :: FVs} -- this is kind of stupid but convenient
           deriving(Eq,Show)
